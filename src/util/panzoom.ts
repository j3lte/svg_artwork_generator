...
extend(Svg, {
    panZoom(options?: Options | false) {
        ...
        const restrictToMargins = (box: Box) => {
            ...
            const { width, height } = fixThis.attr(['width', 'height']) as {
                width: number;
                height: number;
            };
            const preserveAspectRatio =
                fixThis.node.preserveAspectRatio.baseVal;
            ...
            const leftLimit =
                viewbox.width + viewbox.x - left - viewportLeftOffset;
            ...
            const rightLimit =
                viewbox.x + right - box.width - viewportRightOffset;
            // same with top and bottom
            const topLimit =
                viewbox.height + viewbox.y - top - viewportTopOffset;
            const bottomLimit =
                viewbox.y + bottom - box.height - viewportBottomOffset;
        ...
        const wheelZoom = function (ev: WheelEvent) {
            ...
            switch (ev.deltaMode) {
                case 1:
                    normalizedPixelDeltaY =
                        ev.deltaY * wheelZoomDeltaModeLinePixels;
                    break;
                case 2:
                    normalizedPixelDeltaY =
                        ev.deltaY * wheelZoomDeltaModeScreenPixels;
                    break;
                default:
            ...
            if (
                fixThis.dispatch('zoom', { level: lvl, focus: p })
                    .defaultPrevented
            ) {
                return fixThis;
            }
        ...
        const pinchZoomStart = function (ev: TouchEvent) {
            ...
            if (
                fixThis.dispatch('pinchZoomStart', { event: ev })
                    .defaultPrevented
            ) {
                return;
            }
            ...
            fixThis.off(
                'touchstart.panZoom',
                pinchZoomStart as unknown as EventListener
            );
            ...
            on(
                document,
                'touchmove.panZoom',
                pinchZoom as unknown as EventListener,
                fixThis,
                { passive: false }
            );
            on(
                document,
                'touchend.panZoom',
                pinchZoomStop as unknown as EventListener,
                fixThis,
                { passive: false }
            );
        };
        ...
        const panStart = function (ev: Event | PointerEvent) {
            ...
            if (
                isMouse &&
                (ev as PointerEvent).button !== panButton &&
                (ev as PointerEvent).which !== panButton + 1
            ) {
                return;
            }
        ...
        const panning = function (ev: PointerEvent | TouchEvent) {
            ...
            if (
                fixThis.dispatch('panning', { box, event: ev }).defaultPrevented
            ) {
                return;
            }
        ...
        if (doWheelZoom) {
            fixThis.on(
                'wheel.panZoom',
                wheelZoom as unknown as EventListener,
                fixThis,
                { passive: false }
            );
        }
        ...
        if (doPanning) {
            fixThis.on('mousedown.panZoom', panStart, fixThis, {
                passive: false,
            });
        }
